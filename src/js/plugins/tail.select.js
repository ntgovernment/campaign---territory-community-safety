...
const tail = {
    select: function (selector, options = {}) {
        ...
        originalSelects.forEach((originalSelect) => {
            ...
            // Custom: Set select as disabled if original select is disabled
            if (originalSelect.hasAttribute('disabled')) {
                customDropdown.classList.add('tail--disabled');
            };

            // Custom: Optionally use the placeholder attribute on a select element
            var attrPlaceholder = originalSelect.getAttribute('placeholder');

            // Create search input
            const searchInput = document.createElement("input");
            searchInput.type = "text";
            searchInput.classList.add('tail--search');
            searchInput.placeholder = attrPlaceholder || strings.placeholder || "Select an option...";
            ...
            function updateOriginalOptionState(
                ...
                if (!originalSelect.multiple) {
                    if (selectedOptions.length > 0 && searchInput) {
                        if (originalSelect.hasAttribute('preserve-placeholder')) {
                            var firstWord = selectedOptions[0].textContent.replace(/ .*/, '');
                            var formatOption = firstWord.toLowerCase() + ' ' + selectedOptions[0].textContent.split(' ').slice(1).join(' ');

                            searchInput.value = attrPlaceholder + ' ' + formatOption;
                        } else {
